...
class LinkNet(object):
    ...
    def _attention(input_feature, input_size, output_size, name):
        with tf.variable_scope(name_or_scope=name):
            ...
            mask_attention = Net.conv(mask_attention, 3, 3, 64, 1, 1,
            ...
            net_output = Net.conv(attention_feature, 1, 1, output_size[-1] * 2, 1, 1,
                                  biased=False, relu=False, padding='SAME', name='a_conv_o_1')
            net_output = Net.conv(net_output, 1, 1, output_size[-1], 1, 1,
                                  biased=False, relu=False, padding='SAME', name='a_conv_o_2')
    ...
    def build(self):
        ...
        mask_attention_p = tf.image.resize_nearest_neighbor(self.input_mask, block3_shape[:2])  # 初始attention
        block3_mask_feature = tf.multiply(block3, mask_attention_p, name="block3_attention_multiply_add")
        block3_concat = Net.concat([block3_mask_feature, block3_mask_feature], axis=-1, name='attention_3_concat')
        with tf.variable_scope(name_or_scope="attention_3"):
            mask_attention_conv, mask_attention_p, attention_output, attention_output_resize = self._attention(
