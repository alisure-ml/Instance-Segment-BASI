...
class Train(object):
    ...
    def __init__(self, batch_size, input_size, log_dir, data_root_path, train_list, data_path,
        ...
        self.segments, self.attentions = self.net.build()

        # loss
        self.loss, self.loss_segment_all, self.loss_segments = self.cal_loss(self.segments, self.attentions,
                                                                             self.label_seg_placeholder)
    ...
    @staticmethod
    def cal_loss(segments, attentions, label_segment):
        loss_attentions = []
        for attention_index, attention in enumerate(attentions):
            ...
            loss_attentions.append(now_loss_attention * (attention_index + 1))
            pass

        loss_attention_all = tf.add_n(loss_attentions) / len(loss_attentions)
        # 总损失
        loss = loss_attention_all
        return loss, loss_attention_all, loss_attentions
...
if __name__ == '__main__':
    ...
    Train(batch_size=2, input_size=[720, 720], log_dir="./model/attention_u/720_3",
