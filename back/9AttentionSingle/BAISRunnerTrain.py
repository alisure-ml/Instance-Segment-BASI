...
class Train(object):
    ...
    def __init__(self, batch_size, last_pool_size, input_size, log_dir, data_root_path,
                 train_list, data_path, annotation_path, class_path, model_name="model.ckpt", is_test=False):
        ...
        self.loss, self.loss_segment_all, self.loss_class_all, self.loss_segments, self.loss_classes = self.cal_loss(
            self.segments, self.classes, self.label_attention_batch, self.label_classes_placeholder)

        # 当前批次的准确率：accuracy
        self.accuracy_segment = tcm.accuracy(self.pred_segment, self.label_attention_placeholder)
    ...
    @staticmethod
    def cal_loss(segments, classes, label_attention, label_classes):
        loss_segments = []
        for segment_index, segment in enumerate(segments):
            now_loss_segment = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(
                targets=tf.one_hot(tf.reshape(label_attention, [-1, ]), depth=2),
                logits=tf.reshape(segment, [-1, 2]), pos_weight=3))
        ...
        # 总损失
        loss = loss_segment_all + 0.5 * loss_class_all
    ...
    def train(self, save_pred_freq, begin_step=0):
        ...
        for step in range(begin_step, self.num_steps):
            ...
            final_batch_data, final_batch_ann_attention, final_batch_class, batch_data, batch_mask = \
...
if __name__ == '__main__':
    ...
    Train(batch_size=2, last_pool_size=90, input_size=[720, 720], log_dir="./model/attention_u/720",
