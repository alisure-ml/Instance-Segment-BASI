...
class Train(object):
    ...
    def __init__(self, batch_size, input_size, log_dir, data_root_path, train_list, data_path,
        ...
        with tf.name_scope("label"):
            ...
            tf.summary.image("2-segment", tf.cast(self.label_seg_placeholder * 255 // self.num_classes, dtype=tf.uint8))
            pass
        ...
        with tf.name_scope("result"):
            ...
            for segment_index, segment in enumerate(self.segments):
                ...
                for i in range(ii):
                    tf.summary.image("predict-{}-{}".format(segment_index, i),
                                     tf.expand_dims(segment[i] * 255 // self.num_classes, axis=-1))
                    pass
                pass
            pass
    ...
    def cal_loss(self, segments, label_segment):
        ...
        for segment_index, segment in enumerate(segments):
            ...
            now_loss_segment = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(
                targets=tf.one_hot(tf.reshape(now_label_segment, [-1, ]), depth=self.num_classes),
                logits=tf.reshape(segment, [-1, self.num_classes]), pos_weight=1))
...
if __name__ == '__main__':
    ...
    Train(batch_size=2, input_size=[720, 720], log_dir="./model/segment_all/720",
