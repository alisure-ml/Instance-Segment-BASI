...
class Inference(object):
    ...
    def __init__(self, input_size, summary_dir, log_dir, model_name="model.ckpt"):
        ...
        with tf.name_scope("image"):
            ...
            for segment_index, segment in enumerate(self.segments):
                ...
                tf.summary.image("predict-{}".format(segment_index),
    ...
    def inference(self, image_path, image_index, save_path=None):
        ...
        pred_segment_r, summary_now = self.sess.run([self.pred_segment, self.summary_op],
                                                    feed_dict={self.image_placeholder: im_data})
        self.summary_writer.add_summary(summary_now, global_step=image_index)
        s_image = Image.fromarray(np.asarray(np.squeeze(pred_segment_r) * 255 // self.num_classes, dtype=np.uint8))
...
if __name__ == '__main__':
    ...
    is_class = False
    if is_class:
        save_dir = "./output/segment_all/{}/{}_s".format(which_image, which_image)
        inference = InferenceClass(input_size=[720, 720], summary_dir=save_dir, log_dir="./model/segment_all/720")
        ...
        save_dir = "./output/segment_all/{}/{}_ss".format(which_image, which_image)
        inference = Inference(input_size=[720, 720], summary_dir=save_dir, log_dir="./model/segment_all/720")
