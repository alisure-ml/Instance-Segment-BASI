...
class Data(object):
    ...
    def __init__(self, data_list="ImageSets/Segmentation/trainval.txt", data_path="JPEGImages/",
        ...
        # 拆解标签
        self._annotations = self._read_annotation(self._annotation_list, self._class_list, self.image_size)
        # 读取数据
    ...
    def next_batch_train(self):
        ...
        # 根据初始点生成高斯Mask
        batch_mask = [np.expand_dims(self._mask_gaussian(self.image_size, ann[1]), axis=-1) for ann in batch_ann]
        # 标注
        batch_attention = [np.expand_dims(one_ann, 2) for one_ann in batch_ann_attention]
        # 类别
        batch_class = [one_ann[2] for one_ann in batch_ann]

        self._now += 1
        return batch_data, batch_mask, batch_attention, batch_class

    @staticmethod
    def _read_annotation(annotation_list, class_list, image_size):

        all_ann_data = []

        for ann_index, ann_name in enumerate(annotation_list):
            # 读取数据
            class_data = np.asarray(Image.open(class_list[ann_index]).resize((image_size[0], image_size[1])))
...
if __name__ == '__main__':
    ...
    for i in range(20):
        batch_data, batch_mask, batch_attention, batch_class = _data.next_batch_train()
        pass
